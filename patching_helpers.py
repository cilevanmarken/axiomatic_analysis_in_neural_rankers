...
def get_act_patch_block_every(
    ...
    results = torch.zeros(
        3, model.cfg.n_layers, seq_len, device=device, dtype=torch.float32
    )
...
def get_act_patch_attn_head_by_pos(
    ...
    _, seq_len = corrupted_tokens["input_ids"].size()
    results = torch.zeros(
        2, len(layer_head_list), seq_len, device=device, dtype=torch.float32
    )
    ...
    for component_idx, component in enumerate(["z", "pattern"]):
        for i, layer_head in enumerate(layer_head_list):
            ...
            for position in range(seq_len):
                patch_fn = (
                    patch_head_vector_by_pos_pattern
                    if component == "pattern"
                    else patch_head_vector_by_pos
                )
                hook_fn = partial(
